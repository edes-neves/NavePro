...
def _estado_bate(texto, estado, estado_nome):
    if not estado:
        return True
    ...
    valores = {
        _normalizar_texto(estado),
        _normalizar_texto(estado_nome),
        _normalizar_texto(_estado_para_nome(estado)),
    }

    for valor in valores:
        if not valor:
            continue
        if valor in texto_norm:
            return True
    return False


def _cidade_estado_especial(cidade, estado):
    cidade_norm = _normalizar_texto(cidade)
    estado_norm = _normalizar_texto(estado)
    return cidade_norm == 'mundo novo' and estado_norm in {'ms', 'sp', 'go'}


def _resultado_aceitavel_nominatim(item, cidade, estado, estado_nome):
    if not item:
        return False
    ...
    tipo = _normalizar_texto(item.get('type', ''))
    classe = _normalizar_texto(item.get('class', ''))
    ...
    if tipo in {'hamlet', 'suburb', 'neighbourhood', 'isolated_dwelling', 'farm', 'residential', 'locality'}:
        return False
        ...
        for consulta in consultas:
            ...
            if 'results' in geo_data and geo_data['results']:
                ...
                for item in geo_data['results']:
                    ...
                    resultados_com_nome.append(item)
                    if not estado:
                        resultado = item
                        break
                ...
                if estado and resultados_com_nome and not _cidade_estado_especial(cidade, estado):
